...
trait Definitions extends api.StandardDefinitions { self: Forest =>
  ...
  trait ValueClassDefinitions { self: definitions.type =>
    ...
    lazy val symbolCache: mutable.Map[Name, Symbol] =
      mutable.Map(
        nme.scala_ -> RootClass
          .newPackage(NoPosition, nme.scala_)
          .setFlag(FINAL)
      )
    ...
    private def classesMap[T](f: Name => T) =
      symbolsMap(ScalaValueClassesNoUnit, f)
    private def symbolsMap[T](
        syms: List[Symbol],
        f: Name => T
    ): Map[Symbol, T] = (syms zip (syms map (x => f(x.name)))).toMap
    private def symbolsMapFilt[T](
        syms: List[Symbol],
        p: Name => Boolean,
        f: Name => T
    ) = symbolsMap(syms filter (x => p(x.name)), f)
  ...
  object definitions extends AbsDefinitions with ValueClassDefinitions {
    ...
    // top types
    lazy val AnyClass =
      newClass(ScalaPackageClass, tpnme.Any, Nil) setFlag (ABSTRACT)
    lazy val AnyRefClass = newClass(
      ScalaPackageClass,
      tpnme.AnyRef,
      Nil
    ...
    def repeatedParamType(tp: Type) =
      typeRef(NoPrefix, RepeatedParamClass, List(tp))
    ...
    def isCastSymbol(sym: Symbol) =
      sym == Any_asInstanceOf || sym == Object_asInstanceOf

    def isJavaVarArgs(params: List[Symbol]) =
      params.nonEmpty && isJavaRepeatedParamType(params.last.tpe)
    def isScalaVarArgs(params: List[Symbol]) =
      params.nonEmpty && isScalaRepeatedParamType(params.last.tpe)
    def isVarArgsList(params: List[Symbol]) =
      params.nonEmpty && isRepeatedParamType(params.last.tpe)
    def isVarArgTypes(formals: List[Type]) =
    ...
    def byNameParamType(tp: Type) =
      typeRef(NoPrefix, ByNameParamClass, List(tp))
    ...
    def rightType(arg1: Type, arg2: Type): Type =
      typeRef(NoPrefix, RightClass, List(arg1, arg2))
    def leftType(arg1: Type, arg2: Type): Type =
      typeRef(NoPrefix, LeftClass, List(arg1, arg2))
    def symbolType =
      typeRef(SymbolClass.typeConstructor.prefix, SymbolClass, List())
    ...
    private def isTupleType(tp: Type, subtypeOK: Boolean) = tp.normalize match {
      case TypeRef(_, sym, args) if args.nonEmpty =>
        ...
        len <= MaxTupleArity && {
          ...
          (sym == tsym) || (subtypeOK && !tp.isHigherKinded && sym.isSubClass(
            tsym
          ))
        }
      case _ => false
    ...
    def immutableMapType(arg1: Type, arg2: Type) =
      appliedType(ImmutableMapClass.typeConstructor, List(arg1, arg2))
    def mutableMapType(arg1: Type, arg2: Type) =
      appliedType(MutableMapClass.typeConstructor, List(arg1, arg2))
    def immutableSetType(arg: Type) =
      appliedType(ImmutableSetClass.typeConstructor, List(arg))
    def mutableSetType(arg: Type) =
      appliedType(MutableSetClass.typeConstructor, List(arg))
    ...
    def tpEqualsType(arg1: Type, arg2: Type) =
      appliedType(TpEqualsType.typeConstructor, List(arg1, arg2))
    def conformsType(arg1: Type, arg2: Type) =
      appliedType(ConformsType.typeConstructor, List(arg1, arg2))
    def conformsOrViewAsType(arg1: Type, arg2: Type) =
    ...
    def equivType(arg: Type) =
      appliedType(EquivClass.typeConstructor, List(arg))
    def fractionalType(arg: Type) =
      appliedType(FractionalClass.typeConstructor, List(arg))
    def numericType(arg: Type) =
      appliedType(NumericClass.typeConstructor, List(arg))
    def orderedType(arg: Type) =
      appliedType(OrderedClass.typeConstructor, List(arg))
    def orderingType(arg: Type) =
      appliedType(OrderingClass.typeConstructor, List(arg))
    def partialOrderingType(arg: Type) =
    ...
    lazy val uncheckedVarianceClass = getClass(
    ...
    def init(): Unit = {
      ...
      Object_synchronized = newPolyMethodCon(
        ...
        tparam =>
          msym =>
            MethodType(
              msym.newSyntheticValueParams(List(tparam.typeConstructor)),
              tparam.typeConstructor
            )
      ) setFlag FINAL
    }
    ...
    def getModule(fullname: Name): Symbol = getModuleOrClass(
      fullname.toTermName
    )
    ...
    private def newCovariantPolyClass(
        owner: Symbol,
        name: TypeName,
        parent: Symbol => Type
    ): Symbol = {
    ...
    private[Definitions] def newMethod(
        owner: Symbol,
        name: TermName,
        formals: List[Type],
        restpe: Type
      ...
      def flatNameString(sym: Symbol, separator: Char): String =
        ...
        // else if (sym.owner.isPackageClass) sym.javaClassName
          ...
          flatNameString(
            sym.owner,
            separator
          ) + NAME_JOIN_STRING + sym.simpleName
