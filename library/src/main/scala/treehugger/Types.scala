...
trait Types extends api.Types { self: Forest =>
  ...
  abstract class TypeBounds(val lo: Type, val hi: Type, val view: Type, val context: Type)
    ...
    override def safeToString =
      ((if (lo != NothingClass.tpe) List(">:", lo.toString) else Nil) :::
        ...
        (if (context != NothingClass.tpe) List(":", context.toString) else Nil))
        .mkString(" ")
  }
  ...
  abstract class TypeRef(val pre: Type, val sym: Symbol, val args: List[Type]) extends Type {
    ...
    override def typeParams: List[Symbol] =
      if (isHigherKinded) typeParamsDirect else List()
    ...
    private def customToString = this match {
      ...
      case _                                        =>
        if (isFunctionType(this)) {
          ...
          normalize.typeArgs.mkString(
            "(",
            ", ",
            if (hasLength(normalize.typeArgs, 1)) ",)" else ")"
          )
        else if (
          sym.isAliasType && prefixChain.exists(
            _.termSymbol.isSynthetic
          ) && (normalize ne this)
        )
          "" + normalize
        else
  ...
  object TypeVar {
    def unapply(tv: TypeVar): Some[(Type, TypeConstraint)] = Some(
      (tv.origin, tv.constr)
    )
    def apply(origin: Type, constr: TypeConstraint) =
      new TypeVar(origin, constr, List(), List())
    ...
    def apply(tparam: Symbol) =
      new TypeVar(tparam.tpeHK, new TypeConstraint, List(), List())
    def apply(
        origin: Type,
  ...
  class TypeVar(
    ...
    private def levelString =
      "" // if (settings.explaintypes.value) level else ""
...
// Creators ---------------------------------------------------------------
  ...
  def appliedType(tycon: Type, args: List[Type]): Type =
    if (args.isEmpty)
      tycon // @M! `if (args.isEmpty) tycon' is crucial (otherwise we create new types in phases after typer and then they don't get adapted (??))
    else
      tycon match {
        ...
        case TypeBounds(lo, hi, view, context) =>
          TypeBounds(
            appliedType(lo, args),
            appliedType(hi, args),
            appliedType(view, args),
            appliedType(context, args)
          )
        case tv @ TypeVar(_, _)                      => tv.applyArgs(args)
        case AnnotatedType(annots, underlying, self) =>
          AnnotatedType(annots, appliedType(underlying, args), self)
