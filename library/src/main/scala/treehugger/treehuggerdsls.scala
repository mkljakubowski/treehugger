...
trait TreehuggerDSLs { self: Forest =>
  ...
  object treehuggerDSL {
    ...
    def infix(lhs: Tree, op: Name, args: Tree*): Infix =
      Infix(lhs, op, args.toList)
    def infix(lhs: Tree, op: Symbol, args: Tree*): Infix =
      Infix(lhs, op, args.toList)

    def mkInfixOr(lhs: Tree, rhs: Tree) =
      infix(PAREN(lhs), Boolean_or, PAREN(rhs))
    def mkInfixAnd(lhs: Tree, rhs: Tree) =
      infix(PAREN(lhs), Boolean_and, PAREN(rhs))

    class TypeMethods(target: Type) {
      def TYPE_#(sym: Symbol, args: Type*): Type =
        typeRef(target, sym, args: _*)
      def TYPE_#(sym: Symbol, args: Iterable[Type]): Type =
        TYPE_#(sym, args.toList: _*)
    ...
    class TreeMethods(target: Tree) {
      ...
      def INT_&(other: Tree) =
        infix(target, getMember(IntClass, nme.AND), other)
      def INT_>=(other: Tree) =
        infix(target, getMember(IntClass, nme.GE), other)
      def INT_==(other: Tree) =
        infix(target, getMember(IntClass, nme.EQ), other)
      def INT_!=(other: Tree) =
      ...
      def INT_*(other: Tree) =
        infix(target, getMember(IntClass, nme.MUL), other)
      def INT_/(other: Tree) =
        infix(target, getMember(IntClass, nme.DIV), other)
      ...
      def APPLYTYPE(typs: Type*) =
        TypeApply(target, typs.toList map { TypeTree(_) })
      def APPLYTYPE(typs: Iterable[Type]) =
        TypeApply(target, typs.toList map { TypeTree(_) })
      ...
      def INFIXUNAPPLY(name: Name, param0: Tree, params: Tree*) =
        InfixUnApply(target, name, List(param0) ::: params.toList)
      ...
      def withComments(comments: String*): Commented = withComments(
        comments.toList
      )
      def withComments(comments: Iterable[String]): Commented =
        Commented(NoMods, comments.toList, target)
      def withDoc(elems: DocElement*): Commented =
        mkScalaDoc(target, elems.toList)
      def withDoc(comments: Iterable[String], elems: DocElement*): Commented =
        mkScalaDoc(
          target,
          comments.toList.map(s => s: DocElement) ++ elems.toList
        )
    ...
    def INFIX_CHAIN(name: Name, tree: Tree, trees: Tree*): Tree =
      INFIX_CHAIN(name, tree :: trees.toList)
    ...
    def INFIX_CHAIN(sym: Symbol, tree: Tree, trees: Tree*): Tree =
      INFIX_CHAIN(sym, tree :: trees.toList)
    ...
    trait TparamsStart {
      ...
      def withTypeParams(tparam: TypeDef*): this.type = withTypeParams(
        tparam.toList
      )
    ...
    trait SymVODDStart[ResultTreeType <: Tree, RhsTreeType <: Tree]
    ...
    trait DefCreator {
      self: VODDStart[DefDef, Tree] with TparamsStart with VparamssStart =>
      def mkTree(rhs: Tree): DefDef =
        DefDef(mods, name, tparams, vparamss, tpt, rhs)
    }
    trait ProcCreator {
      self: DefStart[ProcDef, Block] with TparamsStart with VparamssStart =>
      def mkTree(rhs: Tree): ProcDef =
        ProcDef(mods, name, tparams, vparamss, rhs)
    }

    class DefSymStart(val sym: Symbol)
        extends SymVODDStart[DefDef, Tree]
        with DefCreator
        with TparamsStart
        with VparamssStart
    ...
    class NoBlockDefSymStart(sym: Symbol) extends DefSymStart(sym) {
      @deprecated(
        "0.4.0",
        "DEF(x) := BLOCK() is no longer supported. Use PROC(x) or DEFINFER(x)."
      )
      def :=(rhs: Block): DefDef = sys.error(
        "DEF(x) := BLOCK() is no longer supported. Use PROC(x) or DEFINFER(x)."
      )
    ...
    trait TreeVODDStart[ResultTreeType <: Tree]
        extends VODDStart[ResultTreeType, Tree]
        with TreeDefStart[ResultTreeType, Tree] {
    ...
    class DefTreeStart(val name: Name)
        extends TreeVODDStart[DefDef]
        with DefCreator
        with TparamsStart
        with VparamssStart

    class ProcTreeStart(val name: Name)
        extends TreeDefStart[ProcDef, Block]
        with ProcCreator
        with TparamsStart
        with VparamssStart
    ...
    class ClassDefStart(val name: TypeName)
        extends TreeDefStart[ClassDef, Tree]
        with TparamsStart
        with ParentsStart {
      ...
      def mkTree(body: List[Tree]): ClassDef = ClassDef(
        mods,
        ctormods,
        name,
        tparams,
        vparams,
        Template(parents, selfDef, body)
      )
    }
    ...
    class ModuleDefStart(val name: TermName)
        extends TreeDefStart[ModuleDef, Tree]
        with ParentsStart {
      ...
      def mkTree(body: List[Tree]): ModuleDef =
        ModuleDef(mods, name, Template(parents, selfDef, body))
    }
    ...
    trait TypeDefStart extends TreeDefStart[TypeDef, Tree] with TparamsStart {
      ...
      def bounds: Tree =
        if (_bounds.isEmpty) EmptyTree
          ...
          TypeTree(
            TypeBounds(
              ((_bounds collect { case LowerTypeBoundsStart(lo) =>
                lo
              }).headOption).getOrElse(NothingClass.tpe),
              ((_bounds collect { case UpperTypeBoundsStart(hi) =>
                hi
              }).headOption).getOrElse(NothingClass.tpe),
              ((_bounds collect { case ViewBoundsStart(trg) =>
                trg
              }).headOption).getOrElse(NothingClass.tpe),
              ((_bounds collect { case ContextBoundsStart(typcon) =>
                typcon
              }).headOption).getOrElse(NothingClass.tpe)
            )
          )
    ...
    class ImportSelectorStart(val name: TermName) {
      def ==>(rename: String): ImportSelector =
        ImportSelector(name, -1, rename, -1)
    }
    ...
    def MATCHERROR(arg: Tree) = Throw(
      New(TypeTree(MatchErrorClass.tpe), List(List(arg)))
    )
    ...
    def THROW(typ: Type, msg: Tree): Throw = Throw(
      New(TypeTree(typ), List(List(msg.TOSTRING)))
    )
    ...
    def VALFROM(name: Name, tp: Type): ForValFromStart =
    ...
    def CASECLASSDEF(name: Name): ClassDefStart =
      CLASSDEF(name) withFlags Flags.CASE
    def CASECLASSDEF(sym: Symbol): ClassDefStart =
      CLASSDEF(sym) withFlags Flags.CASE
    ...
    def CASEOBJECTDEF(name: Name): ModuleDefStart =
      OBJECTDEF(name) withFlags Flags.CASE
    def CASEOBJECTDEF(sym: Symbol): ModuleDefStart =
      OBJECTDEF(sym) withFlags Flags.CASE
    ...
    def RENAME(name: TermName): ImportSelectorStart = new ImportSelectorStart(
      name
    )
    ...
    def INTERP(name: Name, args: Tree*): Interpolated =
    ...
    def IMPORT(pck: Name, selectors: ImportSelector*): Import =
      IMPORT(pck, selectors.toList)
    def IMPORT(pck: Name, selectors: Iterable[ImportSelector]): Import =
      Import(REF(definitions.getClass(pck)), selectors.toList)
    def IMPORT(sym: Symbol, selectors: ImportSelector*): Import =
      IMPORT(sym, selectors.toList)
    def IMPORT(sym: Symbol, selectors: Iterable[ImportSelector]): Import =
      Import(REF(sym), selectors.toList)
    def IMPORT(expr: Tree, selectors: ImportSelector*): Import =
      IMPORT(expr, selectors.toList)
    def IMPORT(expr: Tree, selectors: Iterable[ImportSelector]): Import =
      Import(expr, selectors.toList)
    ...
    def TUPLE(trees: Iterable[Tree], flattenUnary: Boolean = false): Tree =
      trees.toList match {
        ...
        case _                          =>
          mkTuple(
            trees.toList
          ) // Apply(TupleClass(trees.length).companionModule, trees: _*)
      }

    def makeTupleType(trees: List[Tree], flattenUnary: Boolean = false): Tree =
      trees match {
    ...
    def makeStructuralType(trees: List[Tree]): Type = {
      val customString =
        trees map { tree => treeToString(tree) } mkString ("({ ", ", ", " })")
    ...
    implicit def mkTypeMethods(target: Type): TypeMethods = new TypeMethods(
      target
    )
    implicit def mkTypeMethodsFromSymbol(sym: Symbol): TypeMethods =
      new TypeMethods(sym.toType)
    ...
    implicit def mkTreeMethodsFromType(target: Type): TreeMethods =
      new TreeMethods(TypeTree(target))
    implicit def mkTreeMethodsFromSuperStart(target: SuperStart): TreeMethods =
      new TreeMethods(target.tree)
    implicit def mkSymbolMethodsFromSymbol(target: Symbol): SymbolMethods =
      new SymbolMethods(target)

    implicit def mkTreeFromTypeDefStart(tds: TypeDefStart): TypeDef = tds.tree
    implicit def mkSeqTypeDefFromCandidates[A <: TypeDefStart, M[A] <: Iterable[
      A
    ]](in: M[A]): Seq[TypeDef] =
      in.toSeq map { x => (x: TypeDef) }

    implicit def mkTreeFromType(typ: Type): TypeTree = TypeTree(typ)
    implicit def mkSeqTypeTreeFromCandidates[M[A] <: Iterable[A]](
        in: M[Type]
    ): Seq[TypeTree] =
    ...
    implicit def mkTreeFromDefStart[A <: Tree, B <: Tree](
        start: DefStart[A, B]
    ): A = start.empty
